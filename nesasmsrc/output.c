...
putbuffer(void *data, int size)
{
	...
	if (bank >= RESERVED_BANK) {
		...
		if (addr > XXX_BANK_MASK) {
			fatal_error("PROC overflow!");
			return;
		}
	}
	else {
		addr  = loccnt + size + (bank << XXX_BANK_SHIFT);
...
write_srec(char *file, char *ext, int base)
{
	...
	for (i = 0; i <= max_bank; i++) {
		for (j = 0; j < XXX_BANK_SIZE; j++) {
			...
			if (dump) {
				dump = 0;
				addr = base + (i << XXX_BANK_SHIFT) + pos;
