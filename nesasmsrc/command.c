...
do_pseudo(int *ip)
{
	...
	if (pass == LAST_PASS) {
		if (asm_opt[OPT_WARNING]) {
			...
			case P_INCMAP:
				if (bank != old_bank) {
					size = ((bank - old_bank - 1) * XXX_BANK_SIZE) + loccnt;
					if (size) {
...
do_org(int *ip)
{
	...
	case S_DATA:
		...
		page = (value >> XXX_BANK_SHIFT) & 0x07;
		break;
	}

	/* set location counter */
	loccnt = (value & XXX_BANK_MASK);
...
do_incbin(int *ip)
{
	...
	if (((bank << XXX_BANK_SHIFT) + loccnt + size) > rom_limit) {
		fclose(fp);
		error("ROM overflow!");
		return;
	}
	...
	bank  += (loccnt + size) >> XXX_BANK_SHIFT;
	loccnt = (loccnt + size) & XXX_BANK_MASK;
	if (bank > max_bank) {
		if (loccnt)
			max_bank = bank;
		else
	...
	while (fgets(line, 254, fp) != NULL) {
		if (line[0] == 'S') {
			...
			if (type == '2') {
				...
				page   = (addr >> XXX_BANK_SHIFT) & 0x07;
				loccnt = (addr & XXX_BANK_MASK);
...
do_section(int *ip)
{
	...
	/* output line */
	if (pass == LAST_PASS) {
		loadlc(loccnt + (page << XXX_BANK_SHIFT), 1);
		println();
	}
}
