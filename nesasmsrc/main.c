...
main(int argc, char **argv)
{
	...
	/* clear the ROM array */
	memset(rom, 0xFF, XXX_BANK_SIZE * 128);
	memset(map, 0xFF, XXX_BANK_SIZE * 128);
	...
	if (errcnt == 0) {
		...
		if (cd_opt || scd_opt) {
			...
			/* write rom */
			fwrite(rom, XXX_BANK_SIZE, (max_bank + 1), fp);
			fclose(fp);
		}
		...
		/* develo box */
			...
			if ((page + max_bank) < 7)
				/* old format */
				write_srec(out_fname, "mx", page << XXX_BANK_SHIFT);
			else
				/* new format */
	...
	for (i = 0; i <= max_bank; i++) {
		...
		if (nb)			
			printf("BANK% 4i    %20s    %4i/%4i\n",
					i, bank_name[i], nb, XXX_BANK_SIZE - nb);
		else {
			printf("BANK% 4i    %20s       0/XXX_BANK_SIZE\n", i, bank_name[i]);
			continue;
		}

		/* update used/free counters */
		rom_used += nb;
		rom_free += XXX_BANK_SIZE - nb;
		...
		for (;;) {
			...
			for (; addr < XXX_BANK_SIZE; addr++)
				if (map[i][addr] != 0xFF)
					break;

			/* check for end of bank */
			if (addr > (XXX_BANK_SIZE-1))
				break;
